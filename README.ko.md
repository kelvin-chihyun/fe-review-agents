...
AI 코딩 에이전트(Claude Code · Codex · Gemini CLI)용 **스킬 팩**입니다. git diff 또는 변경 파일을 6개 관점(perf·코드 품질·버그·타입·a11y·security)으로 리뷰합니다. 각 관점은 **lens**로 정의합니다. lens는 한 관점에 특화된 리뷰어로, 자기만의 룰셋과 격리된 컨텍스트를 가지고 병렬로 실행됩니다. 결과는 우선순위가 매겨진 하나의 리포트로 합쳐집니다.

기본 프리셋은 _잘 알려진 공인된 프론트엔드 가이드라인_ 을 그대로 따릅니다. 새 lens를 추가하려면 폴더만 하나 만들면 됩니다. 오케스트레이터는 설치된 `lens-*`를 자동 발견합니다.
...
- **Lens 자유롭게 추가** — `skills/lens-<name>/` 폴더 만들면 오케스트레이터 수정 필요없이 다음 호출부터 자동 합류
...
```bash
# Claude Code (primary — orchestrator + 6 lens 전체)
npx fe-review-skills install claude-code

# Codex CLI (lens 만, TOML)
npx fe-review-skills install codex-cli

# Gemini CLI (lens 만, markdown)
...
| `lang`         | `en`        | `en`, `ko`                                                                                            |
| `lenses`       | 설치된 전체 | 콤마 리스트 (`perf` → `lens-react-perf`, `quality` → `lens-code-quality`, 그 외는 lens 이름의 suffix) |
...
`/diff-review` 는 우선순위가 매겨진 단일 리포트를 반환합니다. 라인 범위가 겹친 finding은 하나의 issue로 머지되면서 각 lens의 관점이 나란히 보존됩니다:

```markdown
# 코드 리뷰

**범위:** staged · **파일 수:** 1 · **이슈:** 총 2개 (Critical: 1 / High: 1 / Medium: 0 / Low: 0)
...
**파일:** `src/components/Profile.tsx:4-10` · **심각도:** Critical
**적용 lens:** lens-security, lens-react-perf, lens-bugs

- **lens-security** — Live key 패턴(`sk_live_*`)이 소스에 커밋됨. push protection이 잡는 패턴이라 이미 revoke됐을 가능성 큼.
  권장: 서버 사이드 env로 옮기고, 클라이언트 번들에는 절대 싣지 말 것.
- **lens-react-perf** — 클라이언트 `useEffect` fetch가 render → fetch → render waterfall을 만듦.
  권장: Server Component로 끌어올리고 `bio`를 props로 전달.
- **lens-bugs** — URL에 `userId`가 들어가지만 deps 배열에는 없음 — prop이 바뀌어도 effect가 재실행되지 않아 stale.
  권장: deps에 `userId` 추가 (단, 위 perf 권장을 먼저 적용하는 것이 우선).
...
**파일:** `src/components/Profile.tsx:11-11` · **심각도:** High
**적용 lens:** lens-security

- **lens-security** — 네트워크 응답 HTML을 그대로 렌더. `/api/user` 가 사용자 입력에 영향받을 수 있다면 XSS.
  권장: 서버에서 sanitize하거나 텍스트로 렌더.
```
...
기본 6개로 충분하지 않은 관점이 있다면 (i18n, 모션, 의존성 위생, 디자인 토큰 등), `skills/lens-<name>/SKILL.md` 폴더 하나 만들어 떨어뜨리면 됩니다. 오케스트레이터는 설치된 lens-\* 를 자동 발견하므로 README·`package.json`·오케스트레이터 어느 것도 편집할 필요 없습니다.

전체 가이드: [docs/adding-a-lens.md](docs/adding-a-lens.md) — frontmatter contract, finding JSON 스키마, 룰 카탈로그 형식, 경계 규율(다른 lens와 안 겹치게), 복사-붙여넣기 가능한 SKILL.md 스켈레톤까지 포함.
