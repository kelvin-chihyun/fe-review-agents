...
**`lang=ko|en` is the only optional flag.** Default `ko`. The slash commands parse `lang=...` from `$ARGUMENTS` and pass it through to every reviewer prompt and the synthesizer prompt. Reviewers and synthesizer both support both languages internally. Don't add other flags (no `severity_min`, no `lenses=`, no `triage`) — the `parallel-test` design is intentionally tight.
