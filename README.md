...
[빠른 시작](#빠른-시작) · [렌즈](#렌즈) · [아키텍처](#아키텍처) · [Lens 추가](docs/adding-a-lens.md)

🇰🇷 한국어 · 🇺🇸 [English](./README.en.md)

</div>

fe-review-skills는 git diff를 6개 관점(perf·코드 품질·버그·타입·a11y·security)으로 _동시에_ 리뷰하는 CLI 도구입니다. 각 관점은 **lens**(한 가지 관점만 보는 가이드라인, 격리된 컨텍스트의 sub-agent)로 구현돼 6개가 병렬로 실행되고, 결과는 우선순위가 매겨진 하나의 리포트로 합쳐집니다. 룰셋은 _잘 알려진 공인된 프론트엔드 가이드라인_ 을 그대로 따릅니다. 새 lens를 추가하려면 폴더만 하나 만들면 됩니다 — 오케스트레이터는 설치된 `lens-*`를 자동 발견합니다.
...
- **명령 한 줄로 세팅** — `npx fe-review-skills install <tool>` 로 새 레포에서 즉시 시작. Claude Code가 primary, Codex·Gemini CLI는 lens 단독 호출만 지원
- **Lens 자유롭게 추가** — `skills/lens-<name>/` 폴더 만들면 다음 호출부터 자동 합류. README·오케스트레이터 어느 것도 안 건드림
...
오케스트레이터(`diff-review`)는 Step 0에서 설치된 `lens-*` 를 자동 발견하고, Task 툴로 모두를 fan-out으로 띄웁니다. 머지/정렬은 자기 컨텍스트에서만 합니다. 사용자가 추가한 lens도 같은 경로로 합류합니다.
...
PR 환영합니다. lens 추가는 위 [docs/adding-a-lens.md](docs/adding-a-lens.md) 따라 폴더 하나만 만들면 됩니다. 룰 추가 기준: _"런타임 데이터 없이 lens의 input-mode로 신뢰성 있게 탐지 가능한가?"_ + _"시니어 프론트엔드 리뷰어가 PR에서 지적할 만한가?"_ 둘 다 yes면 추가.
