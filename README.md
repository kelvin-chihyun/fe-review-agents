...
Claude Code용 **멀티-리뷰어 코드 리뷰 플러그인**. git diff 또는 단일 파일을 6가지 관점으로 리뷰합니다. 각 **reviewer**는 자체 룰셋을 가진 단일-목적 에이전트로, 다른 리뷰어들과 함께 한꺼번에 호출됩니다. synthesizer 에이전트가 6개 결과를 하나의 리포트로 합칩니다.

기본 관점은 _업계에서 검증된 프론트엔드 가이드라인_ 을 그대로 따릅니다. 직접 reviewer를 추가하려면 에이전트 파일을 만들고 두 슬래시 커맨드에 등록하세요.
...
> _reviewer_ = 단일-목적 에이전트. 표의 6개는 기본 프리셋이며 직접 추가해도 됩니다. 에이전트 이름은 `reviewer-<name>` 형식
...
- **절대 비용은 작음** — 입력은 diff(또는 단일 파일)로 한정. _PR 한 번의 토큰량 × N_ 이지 _코드베이스 전체 × N_ 이 아닙니다.
...
한 변경에 여러 reviewer가 같은 라인에서 동시에 발동할 수 있습니다.
