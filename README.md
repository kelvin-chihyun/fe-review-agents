...
토큰은 단일 컨텍스트 대비 대략 N배 듭니다. 그 비용을 정당화하는 두 가지:

- **절대 비용은 작음** — 입력은 diff(또는 단일 파일)로 한정. 'PR 한 번의 토큰량 × N'이지 '코드베이스 전체 × N'이 아닙니다.
- **하드 가드** — diff 모드는 필터된 diff가 2,000라인을 넘으면 종료. 비용 폭주를 구조적으로 차단합니다.

그 N배가 사주는 것: 추론 오염과 mode collapse 없는 다관점 커버리지. 단일 컨텍스트로는 프롬프트를 어떻게 쓰든 구조적으로 못 얻는 결과이고, 이게 이 프로젝트의 베팅입니다.

### 한 번에 동시 호출 (병렬 의도)

슬래시 커맨드는 메인 세션에 6개 `Agent` tool_use 블록을 하나의 메시지에 한꺼번에 담아 호출하도록 지시합니다. 실제 동시 실행 여부는 런타임이 결정하므로 wall-time을 약속하지는 않습니다. **다만 아키텍처는 런타임이 허용하는 범위 내에서 동시성을 최대화하도록 구성돼 있습니다.** synthesizer 단계는 6개 리뷰어가 모두 돌아온 뒤 한 번 실행됩니다.

### 두 모드, 같은 fan-out

- **`/fe-review-agents:diff-review`** 는 PR 스타일 리뷰: 관련 diff(staged/unstaged/branch/range) 수집, 프론트엔드 파일만 필터링, diff 텍스트로 6개 리뷰어 호출.
- **`/fe-review-agents:file-review`** 는 단일 파일 딥다이브: 각 리뷰어가 직접 `Read`. PR 열기 전 한 컴포넌트를 새 눈으로 점검할 때 유용.

둘 다 동일한 6개 리뷰어 + 동일한 synthesizer를 거칩니다.
